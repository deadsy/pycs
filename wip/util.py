...
def bitfield_h(val, fieldset):
  """
  return a string of bit field components formatted horizontally
  val: the value to be split into bit fields
  fields: a tuple of (name, msb, lsb, format_function, descr) tuples
  """
  clist = []
  for (name, msb, lsb, fmt, descr) in fieldset:
    mask = ((1 << (msb - lsb + 1)) - 1) << lsb
    x = (val & mask) >> lsb
    if msb == lsb:
      name = '  %s[%d]' % (name, lsb)
    else:
      name = '  %s[%d:%d]' % (name, msb, lsb)
    val_name = ''
    if fmt is not None:
      if callable(fmt):
        val_name = fmt(x)
        ...
        val_name = fmt.get(x, '?')
    val_str = (': 0x%x %s' % (x, val_name), ': %d %s' % (x, val_name))[x < 10]
    clist.append([name, val_str, '', descr])
  return display_cols(clist)
