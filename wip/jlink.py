...
class JLink(object):
  ...
  def hw_jtag_write(self, tms, tdi, tdo=None):
    """Write and read a bitstream to the JTAG interface"""
    n = len(tms)
    assert len(tdi) == n
    cmd = [self.hw_jtag_cmd, 0, n & 0xff, (n >> 8) & 0xff]
    cmd.extend(tms.get_bytes())
    cmd.extend(tdi.get_bytes())
    self.write_data(Array('B', cmd))
    nbytes = (n + 7) >> 3
    assert nbytes % 64
  ...
  def _wrap_api(self):
    ...
    import inspect
    args, _, _, _ = inspect.getargspec(usb.core.Device.read)
...
x_to_idle = bits.from_tuple('011111')  # any state -> run-test/idle
idle_to_irshift = bits.from_tuple('0011') # run-test/idle -> shift-ir
idle_to_drshift = bits.from_tuple('001')  # run-test/idle -> shift-dr
xshift_to_idle = bits.from_tuple('011')   # shift-x -> run-test/idle
...
class jtag(object):
  ...
  def test_reset(self, delay=0.1):
    """pulse the test reset line"""
    self.jlink.trst(False)
    time.sleep(delay)
    self.jlink.trst(True)

  def system_reset(self, delay=0.1):
    """pulse the system reset line"""
    self.jlink.srst(False)
    time.sleep(delay)
    self.jlink.srst(True)

  def tap_reset(self):
    """reset the TAP state machine"""
    tdi = bits.zeroes(len(x_to_idle))
    self.jlink.hw_jtag_write(x_to_idle, tdi, None)

  def scan_ir(self, tdi, tdo=None):
    """scan bits through the IR chain"""
    tms = bits.null().tail(idle_to_irshift).tail0(len(tdi) - 1).tail(xshift_to_idle)
    tdi = bits.zeroes(len(idle_to_irshift)).tail(tdi).tail0(len(xshift_to_idle) - 1)
    #print('scan_ir tms %s' % tms)
    #print('scan_ir tdi %s' % tdi)
    self.jlink.hw_jtag_write(tms, tdi, tdo)
    if tdo is not None:
      tdo.drop_head(len(idle_to_irshift))
      tdo.drop_tail(len(xshift_to_idle) - 1)
      #print('scan_ir tdo %s' % tdo)

  def scan_dr(self, tdi, tdo=None):
    """scan bits through the DR chain"""
    tms = bits.null().tail(idle_to_drshift).tail0(len(tdi) - 1).tail(xshift_to_idle)
    tdi = bits.zeroes(len(idle_to_drshift)).tail(tdi).tail0(len(xshift_to_idle) - 1)
    #print('scan_dr tms %s' % tms)
    #print('scan_dr tdi %s' % tdi)
    self.jlink.hw_jtag_write(tms, tdi, tdo)
    if tdo is not None:
      tdo.drop_head(len(idle_to_drshift))
      tdo.drop_tail(len(xshift_to_idle) - 1)
      #print('scan_dr tdo %s' % tdo)
